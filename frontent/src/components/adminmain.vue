...
export default {
  ...
  methods: {
    datacate() {
      ...
      axios
        .get("http://127.0.0.1:5000/category/ADMIN", {
          headers: {
  ...
  <div>
    ...
    <!-- <div v-for="cate in datacategory" :key="cate.Category_id">
      <div class="cats">
        ...
        <button @click="deletcats(cate)">Delete</button>
      </div>
      <div
        v-for="prod in filterProduct(cate.Category_id)"
        :key="prod.Product_id">
        <div class="prod">
          <div class="item">{{ prod.Productname }}</div>
          <div class="item">{{ prod.Qut }}</div>
          <div class="item">{{ prod.Unit }}</div>
          <div class="item">{{ prod.price }}</div>
          <div class="item">{{ prod.Exp }}</div>
          <div class="item">{{ prod.MFG }}</div>
          <div class="item">
            <button @click="deletprod($prod)">Delete</button>
          </div>
        </div>
      </div>
    </div> -->
